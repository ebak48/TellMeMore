...
function buildFallbackInsight(name, mode, signals, path) {
  ...
  const modeLabel = mode === 'friends' ? 'arkadaşları' : 'çevresi';
  return {
    howPeopleSeeYou: `${name}'ı tanıyanlar, onu ${path === 'A' ? 'yakın ve güvenilir' : 'ilginç ama gizemli'} olarak tanımlıyor.`,
    whatYouDontRealize: magneticText,
    yourSocialEnergy: magneticScore >= 2 ? `${name}'ın varlığı ortamda fark ediliyor — bunu her zaman göstermiyor olsa da.` : `${name} çevresini zaman içinde etkiliyor — ilk izlenim tam resmini yansıtmıyor.`,
    contradiction: contradictionText,
    whatKeepsShowing: `${modeLabel.charAt(0).toUpperCase() + modeLabel.slice(1)} aynı şeyi tekrar ediyor. Bu pattern tesadüf değil.`,
